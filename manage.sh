#!/usr/bin/env bash
#
# manage.sh — top-level project tool for JADB.
#
# Combines release (option 1), tag management (option 2), set-version
# (option 3), and view-version (option 4) into a single interactive entry
# point.
#
...
# scripts/_version_files.sh is sourced relative to this script's own
# location.
#
# Note on JADB vs NiceSSH: NiceSSH has a 4th version source
# (release-please-manifest.json, an artifact of using release-please).
# JADB does not use release-please, so this script targets only the 3
# sources: package.json, src-tauri/tauri.conf.json, src-tauri/Cargo.toml.

set -euo pipefail

# Always run from the repo root, so jq/awk find the 3 source files by relative path.
if ! REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null)"; then
  echo "::error::manage.sh must be run from inside a git repository." >&2
  echo "  Current dir: $(pwd)" >&2
  echo "  Try: cd /path/to/JADB && bash manage.sh" >&2
  exit 1
fi
cd "$REPO_ROOT"
...
═══════════════════════════════════════
  JADB 项目工具
═══════════════════════════════════════
  1) 打 tag 发版    (改 3 文件到 next version + commit + push main + push tag)
  2) Tag 管理       (列 TAG 本地+远端 / 删本地 / 删远端 — 删必须 CONFIRM=1; q 返回主菜单)
  3) 修改版本号     (改 3 文件到 X.Y.Z 或 PATCH; 不 commit, 你自己提交)
  4) 查看版本号     (只读打印 3 文件当前 version + 下一个 next 预览)
  5) 退出
═══════════════════════════════════════
MENU
}
...
opt_release() {
  ...
  if [ -z "$major_minor" ] || ! [[ "$major_minor" =~ ^[0-9]+\.[0-9]+$ ]]; then
    ...
    echo "  Use option 3 first to set a version, or pass: MAJOR_MINOR=0.2 $0" >&2
    return 1
  fi
  patch="$(commit_count)"
  ...
  local old_pkg old_tauri old_cargo
...
Press Enter to release, 'd' to print a diff, 'q' to abort.
EOF
  printf "> "
  read -r ans
  ...
  case "$ans" in
    d|D)
      echo ""
      echo "── Roll-forward preview (will be applied on next 'Enter') ──"
      local tmp_old
      tmp_old="$(mktemp -d)"
      cp package.json              "$tmp_old/"
      cp src-tauri/tauri.conf.json "$tmp_old/"
      cp src-tauri/Cargo.toml      "$tmp_old/"

      rewrite_3_files "$version"
      trap 'rollback_3_files' RETURN

      diff -u "$tmp_old/package.json"              package.json              || true
      diff -u "$tmp_old/src-tauri/tauri.conf.json" src-tauri/tauri.conf.json || true
      diff -u "$tmp_old/src-tauri/Cargo.toml"      src-tauri/Cargo.toml      || true
      ...
      echo ""
      echo "Nothing has been written. Re-run option 1 when ready."
      return 0
      ;;
    q|Q)
      echo "Aborted."
    ...
    "")
      ...
      ;;
    *)
      echo "Aborted; unknown response '${ans}'."
      return 1
      ;;
  esac

  # Release path
  trap 'echo "::error::Release failed mid-flight; rolling back." >&2; rollback_3_files' ERR
  rewrite_3_files "$version"
  trap - ERR
  ...
  echo ""
  echo "✅ Released ${tag}"
  echo "→ Build & Publish Release (CI) will run on the new tag."
  echo "→ Watch: https://github.com/${GITHUB_REPOSITORY:-e9ab98/JADB}/actions"
}
...
opt_tags() {
  ...
  echo ""
  echo "  1) 列 TAG (本地 + 远端)"
  echo "  2) 删本地 v* tag     (需要 CONFIRM=1)"
  echo "  3) 删远端 v* tag     (需要 CONFIRM=1)"
  echo "  q) 返回主菜单"
  printf "> "
  read -r sub
...
list_remote_tags() {
  ...
  local remote
  remote="$(git ls-remote --tags origin 2>/dev/null | sed -n 's/.*refs\/tags\///p' | grep '^v' | sort -V -r || true)"
  if [ -z "$remote" ]; then
...
opt_set_version() {
  ...
  echo "── Option 3: 修改版本号 ──"
  echo ""
  echo "  This rewrites the 3 source files to your target version."
  echo "  It does NOT commit, push, or tag — you handle that yourself."
  echo ""
  echo "  Formats:"
  echo "    X.Y.Z   literal (e.g. 0.3.5, 1.0.0, 2.1.0)"
  echo "    PATCH   use current major.minor from the 3 files; patch = commit count"
  echo ""
  ...
  trap 'echo "::error::set-version failed mid-flight; rolling back." >&2; rollback_3_files' ERR
  rewrite_3_files "$version"
  trap - ERR

  echo ""
  echo "✅ Rewrote 3 files to ${version} (not committed; review with 'git diff' then commit yourself)."
}
